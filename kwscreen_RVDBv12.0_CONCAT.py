...
import os
import sys
currentvs=sys.argv[1]
date=sys.argv[2]
screentype=sys.argv[3]
seqtype=sys.argv[4]
if seqtype=='tpa':
    istpa=True
else:
    istpa=False
homedir='F:\\RVDBv'+currentvs
gbdir=homedir+'\\GenBank_raw_data_'+date
poskw_outdir=gbdir+'\\poskw_out_'+date
sizemirna_outdir=gbdir+'\\sizemirna_out_'+date
negkw_outdir=gbdir+'\\negkw_out_'+date
scriptdir=gbdir+'\\scripts'
logdir=gbdir+'\\log'
refseqdir=homedir+'\\Refseq_raw_data_'+date
input_filenames=[infn.strip() for infn in sys.stdin.readlines()]
...
outf=open(gbdir+'\\log\\'+'log_unzip_filenames.txt','w')
outf.write('screentype: '+screentype+'\n'+'date: '+date+'\n'+'currentvs: '+currentvs+'\n')
