...
##################################################################################################
########## Negative keywords added 5/8/2018                                            ###########
##################################################################################################
negkwsnew=['pequenovirus','thermococcus','streptococcus','bacillus','elongation factor','bindingprotein','binding protein','(-)inhibitory protein','(-)inhibitory ptotein',
           'implicated in','restriction factor','LINE reverse transcriptase','seminal vesicle','Loa Loa 15kDa','Mouse alpha-1 type-I collagen',
           'endogenous ligand','polymerase lambda','proteinase inhibitor','restriction factor','beta-tubulin','beta tubulin','small outer capsid protein',
           'cell envelope','5.8S rRNA','photosystem (psb)','cyanomyovirus (T4)','cyanomyovirus','endogenous plasmid','proteasome','granzyme','angiogenesis',
           'pre-insertion region','transfer RNA','virus induced','virus-induced','Chlamydia trachomatis integrase gene','rDNA insertion element',
           'proteosome','IAP-inhibitory protein','VHH mRNA','antimicrobial peptide','endogenous tripeptide']

### Variables for adding negkws in future (please note date as a comment)
negkwsnew2=[]
negkwsnew3=[]
negkwsnew4=[]
negkwsnew5=[]

negkws.extend(negkwsnew)
negkws.extend(negkwsnew2)
negkws.extend(negkwsnew3)
negkws.extend(negkwsnew4)
negkws.extend(negkwsnew5)
...
def negative_rules(description_s):
    ...
    if ' voucher ' in ds:
        if not 'retrotransposon' in ds:
            nonviral.append('voucher - retrotransposon')
    if ' beta globin' in ds:
        if not 'long terminal repeat' in ds and not 'retrovirus like' in ds:
            nonviral.append('beta globin - long terminal repeat / retrovirus like')
...
if istpa:
    poskw_outdir=tpadir+'\\poskw_out_'+date
    sizemirna_outdir=tpadir+'\\sizemirna_out_'+date
    negkw_outdir=tpadir+'\\negkw_out_'+date
    scriptdir=tpadir+'\\scripts'
    logdir=tpadir+'\\log'
else:
